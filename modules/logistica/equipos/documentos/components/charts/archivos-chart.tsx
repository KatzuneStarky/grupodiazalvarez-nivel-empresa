...
import { useStorageUsagePerSubfolder } from "@/hooks/use-storage-per-document-usage";
...
const ArchivosChart = () => {
    const { storageData, loading, error }
        = useStorageUsagePerSubfolder("equipos", 60000);

    const totalArchivos = storageData
        ? Object.values(storageData).reduce((acc, sizes) => acc + sizes.archivos, 0)
        : 0;

    const totalArchivosVencimiento = storageData
        ? Object.values(storageData).reduce((acc, sizes) => acc + sizes.archivosVencimiento, 0)
        : 0;

    const totalCertificado = storageData
        ? Object.values(storageData).reduce((acc, sizes) => acc + sizes.certificado, 0)
        : 0;

    const totalUsed = totalArchivos + totalArchivosVencimiento + totalCertificado;
    const remainingStorage = MAX_STORAGE - totalUsed;
